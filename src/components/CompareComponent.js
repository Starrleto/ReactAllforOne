...
import { greaterLess } from '../services/DataServices';
...
const getResult = async() => {
    ...
    const promise = await greaterLess(numOne, numTwo);
    if(promise[1] != undefined)
        setResult(promise[0]+", and "+promise[1]);
    else
        setResult(promise);
}
  ...
  return (
    ...
    <Navbar expand="lg" className="flex funny-navbar">
        ...
        <h1 className="main-title-2">Greater or Less</h1>
        <p className="subs separate-bottom">Enter two numbers to compare them.</p>
            <Form>
